...
var reverseArr = function(array) {
  ...
  var lastLetter = array[array.length - 1];
  return [lastLetter].concat(reverseArr(array.slice(0, -1)));
};
...
var replaceKeysInObj = function(obj, oldKey, newKey) {
  let rReplace = (keys) => {
    ...
    //recursing on nested objects
    obj[keys[0]] = (typeof obj[keys[0]] === 'object') ?
      replaceKeysInObj(obj[keys[0]], oldKey, newKey) : obj[keys[0]];
    //replacing the key
    if (keys[0] === oldKey) {
      obj[newKey] = obj[oldKey]
      delete obj[oldKey];
    }
    rReplace(keys.slice(1));
  }
  rReplace(Object.keys(obj));
  return obj;
};
...
var fibonacci = function(n) {
  ...
  return fibonacci(n - 1).concat([fib[fib.length - 1] + fib[fib.length - 2]])
};
...
var flatten = function(array) {
  ...
  return (Array.isArray(array[0]) ?
    flatten(array[0]) : [array[0]]).concat(flatten(array.slice(1)));
};
